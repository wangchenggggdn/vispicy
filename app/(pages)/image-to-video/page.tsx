...
import { VideoIcon, Download, Loader2, Upload, Coins } from 'lucide-react';
...
export default function ImageToVideoPage() {
  ...
  // 获取Image to Video模型列表
  const { data: models } = useSWR<AIModel[]>('/api/models?type=image2video', fetcher);
  ...
  const calculatedPrice = useMemo(() => {
    ...
    const originalPrice = calculatePrice('image2video', selectedModel, modelParams);
    const discountedPrice = calculateDiscountedPrice(originalPrice, 'video');

    console.log('[Image-to-Video] Original price:', originalPrice);
    console.log('[Image-to-Video] Discounted price:', discountedPrice);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `vicraft-img2vid-${Date.now()}.mp4`;
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Upload static images, AI transforms them into dynamic video content</p>
        </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 text-center hover:border-indigo-400 transition">
                  {sourceImagePreview ? (
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Motion Description (Optional)</label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder="Describe the motion effects you want, e.g., Camera slowly zooms in, corgi dog gently paddling in water..."
                  className="w-full h-32 px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
                  disabled={loading}
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
              >
                {loading ? (
                  <>
                  ...
                  <>
                    ...
                    <span>生成视频 ({calculatedPrice === 0 ? '免费' : (
                      <>
                        {calculatedPrice}
              ...
              {!session && (
                <p className="text-center text-sm text-gray-500">
                  请先<Link href="/api/auth/signin" className="text-indigo-600 hover:underline">登录</Link>后使用
                </p>
              )}
            </div>
          </div>
