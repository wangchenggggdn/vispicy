...
import { createJob, pollJob } from '@/lib/shortapi';
...
export async function POST(request: Request) {
  try {
    ...
    console.log('[Text-to-Image] Request body:', JSON.stringify(body, null, 2));
    ...
    console.log('[Text-to-Image] Creating job with model:', model);
    console.log('[Text-to-Image] Args:', JSON.stringify(args, null, 2));
    ...
    // 计算原价
    const originalPrice = calculatePrice('text2image', model, args);
    console.log('[Text-to-Image] Original price:', originalPrice, 'coins');
    ...
    if (!models || models.length === 0) {
      console.error('[Text-to-Image] No models available in database');
    ...
    if (!selectedModel) {
      console.error('[Text-to-Image] Model not found:', model, 'Available models:', models.map((m: any) => m.shortapi));
    ...
    if (selectedModel.parameters) {
      try {
        ...
        console.error('[Text-to-Image] Failed to parse parameters:', e);
      }
    }
    ...
    try {
      ...
      console.log('[Text-to-Image] Generation history created');
    } catch (error) {
      console.error('[Text-to-Image] Failed to create generation history:', error);
      // 不影响主流程，继续返回
    }
